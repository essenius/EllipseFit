...
namespace EllipseFitTest {
	...
	TEST(CartesianEllipseTest, RotatedTranslatedEllipseTest) {
		...
		expectCoordinatesEqual(Coordinate{ 11,10 }, center, "Center");
		const auto radius = cartesianEllipse.getRadius();
		expectCoordinatesEqual(Coordinate{ 6, 4 }, radius, "Radius");
		const auto angle = cartesianEllipse.getAngle();
		expectAnglesEqual(Angle{ -M_PI / 4 }, angle, "Angle");
	}
	...
	TEST(CartesianEllipseTest, PlainEllipseTest) {
		...
		const auto cartesianEllipse = CartesianEllipse(quadraticEllipse);
		const auto center = cartesianEllipse.getCenter();
		expectCoordinatesEqual(Coordinate{ 0, 0 }, center, "Center");
		const auto radius = cartesianEllipse.getRadius();
		expectCoordinatesEqual(Coordinate{ 2, 1 }, radius, "Radius");
		const auto angle = cartesianEllipse.getAngle();
		expectAnglesEqual(Angle{ 0 }, angle, "Angle");
	}
	...
	TEST(CartesianEllipseTest, TranslatedEllipseTest) {
		...
		const auto cartesianEllipse = CartesianEllipse(quadraticEllipse);
		const auto center = cartesianEllipse.getCenter();
		expectCoordinatesEqual(Coordinate{ -5, -4 }, center, "Center");
		const auto radius = cartesianEllipse.getRadius();
		expectCoordinatesEqual(Coordinate{ 3, 2 }, radius, "Radius");
		const auto angle = cartesianEllipse.getAngle();
		expectAnglesEqual(Angle{ 0 }, angle, "Angle");
	}
	...
	TEST(CartesianEllipseTest, RotatedEllipseTest) {
		...
		expectCoordinatesEqual(Coordinate{ 3, 2 }, radius, "Radius");
		const auto angle = cartesianEllipse.getAngle();
		expectAnglesEqual(Angle{ 0.4488 }, angle, "Angle");
	}
	...
	TEST(CartesianEllipseTest, Rotated45EllipseTest) {
		...
		expectCoordinatesEqual(Coordinate{ 0, 0 }, center, "Center");
		const auto radius = cartesianEllipse.getRadius();
		expectCoordinatesEqual(Coordinate{ 10, 9 }, radius, "Radius");
		const auto angle = cartesianEllipse.getAngle();
		expectAnglesEqual(Angle{ M_PI / 4 }, angle, "Angle");
	}
	...
	TEST(CartesianEllipseTest, PointOnEllipseForTest) {
		...
		const auto cartesianEllipse = CartesianEllipse(quadraticEllipse);
		expectAnglesEqual(Angle{ -M_PI / 6 }, cartesianEllipse.getAngle(), "Angle");
		constexpr auto Origin = Coordinate{ 0, 0 };
		const auto originMap = cartesianEllipse.getPointOnEllipseClosestTo(Origin);
		expectCoordinatesEqual(Coordinate{ -16.9761709, -36.377509 }, originMap, "Origin mapped on ellipse");

		expectDoubleEqual(40.143662, cartesianEllipse.getDistanceFrom(Origin), "Distance from origin");
		...
		expectCoordinatesEqual(Coordinate{ -28.455996, -35.680005 }, pointInEllipseMap, "Point inside ellipse mapped on ellipse");
		expectDoubleEqual(5.532293, cartesianEllipse.getDistanceFrom(POINT_IN_ELLIPSE), "Distance from point inside ellipse");
	}
	...
	TEST(CartesianEllipseTest, CircleCircumferenceTest) {
		...
		expectDoubleEqual(2 * M_PI * cartesianCircle.getRadius().x, cartesianCircle.getCircumference(), "Circumference");
		// now we're at it, see if angle doesn't crash and burn
		const auto angle = cartesianCircle.getAngle();
		expectAnglesEqual(Angle{ M_PI / 2 }, angle, "Circle angle");
	}
	...
	TEST(CartesianEllipseTest, EllipseCircumferenceTest) {
		...
		expectDoubleEqual(66.01085, cartesianCircle.getCircumference(), "Circumference");
	}
